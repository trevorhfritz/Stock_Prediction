...
# ── Feature Engineering ──────────────────────────────────────────────────
# Computes the 6 derived Feature Set B columns from the 19 raw user inputs.
...
# ── Load Pipeline from S3 (cached) ──────────────────────────────────────
# Mirrors load_pipeline() in HW6. Cached so it only downloads once per session.
...
def load_pipeline(_session, bucket):
    ...
    s3_client.download_file(Bucket=bucket, Key=key, Filename=tar_path)

    with tarfile.open(tar_path, "r:gz") as tar:
        tar.extractall(path=tmp_dir)
        joblib_file = [f for f in tar.getnames() if f.endswith('.joblib')][0]

    return joblib.load(os.path.join(tmp_dir, joblib_file))


# ── Load SHAP Explainer from S3 (cached) ────────────────────────────────
# Mirrors load_shap_explainer() in HW6. Cached so it only downloads once.

@st.cache_resource
def load_shap_explainer(_session, bucket):
    s3_client      = _session.client('s3')
    explainer_name = MODEL_INFO["explainer"]
    s3_key         = f"explainer/{explainer_name}"
...
def call_model_api(input_df: pd.DataFrame):
    ...
    try:
        ...
        # output_fn returns JSON: [[pred_class, pred_prob], ...]
